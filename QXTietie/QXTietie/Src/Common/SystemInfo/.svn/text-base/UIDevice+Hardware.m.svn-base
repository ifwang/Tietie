...
- (NSString *) decodeMacAddress
{
    ...
    NSLog(@"%@", [NSString stringWithFormat:@"%02X:%02X:%02X:%02X:%02X:%02X", *ptr, *(ptr+1), *(ptr+2), *(ptr+3), *(ptr+4), *(ptr+5)]);
    unsigned int table[12][16] = {
    ...
    unsigned int outint[12];
    NSMutableString *outputStr = [[NSMutableString alloc] initWithCapacity:12];
    for (int i = 0; i < 6; i++)
    {
        for (int j = 0 ; j < 2; j++) {
            unsigned int mm = *(ptr + i);
            unsigned int ppmm = 0;
            //            NSLog(@"%X", mm);
            if (j == 0) {
                ppmm = mm >> 4;
            ...
            //            NSLog(@"%X", ppmm);
            //            NSLog(@"%X", table[i * 2 + j][ppmm]);
            outint[i * 2 + j] = ppmm;
            [outputStr appendFormat:@"%X", table[i * 2 + j][ppmm]];
        }
    }
    NSLog(@"%@", outputStr);
...
- (UIDeviceScreenFamily)screenSize
{
    CGSize currentScreenSize = [[UIScreen mainScreen] bounds].size;
    NSInteger currentHeight = currentScreenSize.height;
    
    UIDeviceScreenFamily screenType = ThreePointFive;
    switch (currentHeight) {
        case 480:
            screenType =  ThreePointFive;
            break;
            
        case 568:
            screenType = FourPointO;
            break;
